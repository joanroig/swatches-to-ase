...
const normalizeRgb = (rgb: number[]): [number, number, number] => {
  ...
  if (max > 1.5) {
    return [clamp(rgb[0] / 255), clamp(rgb[1] / 255), clamp(rgb[2] / 255)];
  }
...
const nameColor = (rgb: [number, number, number], format: string) => {
  const hex = `#${convert.rgb.hex([Math.round(rgb[0] * 255), Math.round(rgb[1] * 255), Math.round(rgb[2] * 255)])}`;
...
const parseAse = (data: ArrayBuffer | Uint8Array): Palette => {
  ...
  for (const entry of decoded.colors ?? []) {
    ...
    if (model === "RGB") {
      ...
      const converted = convert.cmyk.rgb(scaled[0] ?? 0, scaled[1] ?? 0, scaled[2] ?? 0, scaled[3] ?? 0);
      rgb = normalizeRgb(converted);
    } else if (model === "LAB") {
      const max = Math.max(...values, 0);
      const scaled = max <= 1 ? [values[0] * 100, values[1] * 100, values[2] * 100] : values;
      const converted = convert.lab.rgb(scaled[0] ?? 0, scaled[1] ?? 0, scaled[2] ?? 0);
      rgb = normalizeRgb(converted);
    } else if (model === "GRAY") {
...
const parseSwatches = async (data: ArrayBuffer | Uint8Array, options: PaletteImportOptions): Promise<Palette> => {
...
const withBlackWhite = (palette: Palette, options: PaletteImportOptions): Palette => {
  if (!options.addBlackWhite) {
    return palette;
  }
  const hasColor = (target: [number, number, number]) =>
    palette.colors.some((color) => color.rgb[0] === target[0] && color.rgb[1] === target[1] && color.rgb[2] === target[2]);
...
export const readPaletteFile = async (
  ...
  if (ext === "swatches") {
    const palette = await parseSwatches(
      data instanceof Uint8Array || data instanceof ArrayBuffer ? data : new TextEncoder().encode(data),
      options,
    );
    return withBlackWhite(palette, options);
  }
  if (ext === "ase") {
    const palette = parseAse(data instanceof Uint8Array || data instanceof ArrayBuffer ? data : new TextEncoder().encode(data));
    return withBlackWhite(palette, options);
  }
  if (ext === "gpl") {
...
export const exportPaletteToSwatches = async (palette: Palette): Promise<Uint8Array> => {
  const entries: Array<[number[], string]> = palette.colors.map((color) => {
    const rgb: [number, number, number] = [Math.round(color.rgb[0] * 255), Math.round(color.rgb[1] * 255), Math.round(color.rgb[2] * 255)];
...
export const exportPaletteToGpl = (palette: Palette): string => {
  ...
  for (const color of palette.colors) {
    const rgb = [Math.round(color.rgb[0] * 255), Math.round(color.rgb[1] * 255), Math.round(color.rgb[2] * 255)];
