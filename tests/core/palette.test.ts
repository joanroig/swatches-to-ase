...
import { exportPaletteToAse, exportPaletteToGpl, exportPaletteToSwatches, readPaletteFile } from "../../src/core/palette.js";

const fixturesDir = path.resolve("tests/fixtures");
const sampleInputs = ["Kitchen_Plant.swatches", "Midnight_Produce.swatches", "Wild_Moments.swatches"];
...
test("convertSwatchesToAse produces stable ASE output", async () => {
  for (const fileName of sampleInputs) {
    const input = fs.readFileSync(path.resolve("examples/palette-in", fileName));
    const expected = fs.readFileSync(path.join(fixturesDir, "ase", fileName.replace(".swatches", ".ase")));
    const actual = await convertSwatchesToAse(input, options);
    assert.equal(Buffer.compare(Buffer.from(actual), expected), 0, `${fileName} did not match fixture`);
  }
});

test("readPaletteFile preserves swatches metadata", async () => {
  const input = fs.readFileSync(path.resolve("examples/palette-in", "Kitchen_Plant.swatches"));
...
test("palette exports remain deterministic across formats", async () => {
  ...
  const aseExpected = fs.readFileSync(path.join(fixturesDir, "exports", "Kitchen_Plant.ase"));
  const gplExpected = fs.readFileSync(path.join(fixturesDir, "exports", "Kitchen_Plant.gpl"), "utf8").replace(/\r\n/g, "\n");
  ...
  const decodedColors = decoded.colors.filter(Boolean) as Array<[[number, number, number], string]>;
  ...
  decodedColors.forEach(([hsv], index) => {
    ...
    const expected = paletteColors[index].rgb.map((value) => Math.round(value * 255));
    rgb.forEach((channel, channelIndex) => {
      assert.ok(Math.abs(channel - expected[channelIndex]) <= 2, `Swatches color ${index + 1} channel ${channelIndex + 1} mismatch`);
    });
  });
});

test("addBlackWhite prepends black and white for ase and gpl imports", async () => {
  const aseInput = fs.readFileSync(path.join(fixturesDir, "ase", "Kitchen_Plant.ase"));
  const gplInput = fs.readFileSync(path.join(fixturesDir, "exports", "Kitchen_Plant.gpl"), "utf8");
  ...
  const asePalette = await readPaletteFile(aseInput, "Kitchen_Plant.ase", importOptions);
  const gplPalette = await readPaletteFile(gplInput, "Kitchen_Plant.gpl", importOptions);
