...
/*
 * Nothing arrives twice on a reload.
 *
 * The panel's entry animation used to be on every active section, so it also ran on first paint:
 * the app was revealed, and then its contents faded up underneath a loading screen that was still
 * dissolving. Two separate arrivals, and the icons were the part you noticed doing it.
 */
test("no entry animation plays while the app is first revealed", async ({ page }) => {
  await page.addInitScript(() => {
    const w = window as unknown as { __entries: string[] };
    w.__entries = [];
    const attach = () =>
  ...
  expect(await page.evaluate(() => (window as unknown as { __entries: string[] }).__entries)).toEqual([]);

  // ...but a real change of view still animates the panel coming in.
  await page.locator(".nav-item[data-view-target='playground']").first().click();
  await expect.poll(() => page.evaluate(() => (window as unknown as { __entries: string[] }).__entries)).toContain("view-enter");
});
